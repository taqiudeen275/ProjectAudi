...
export function createRipple(
  ...
  function render() {
    ...
    const uniform = (name: string) => uniforms[name] ?? null;
    ...
    gl!.uniform4fv(uniform("uRipples"), rippleData);
    gl!.uniform1i(uniform("uCount"), ripples.length);
    gl!.uniform1f(uniform("uSpeed"), BASE_SPEED * Math.max(config.speed, 0.05) * dpr);
    gl!.uniform1f(uniform("uWavelength"), Math.max(config.wavelength, 4) * dpr);
    gl!.uniform1f(
      uniform("uWidth"),
      Math.max(config.wavelength, 4) * Math.max(config.rings, 1) * 0.5 * dpr,
    );
    gl!.uniform1f(uniform("uDecay"), Math.max(config.decay, 0.05));
    gl!.uniform1f(uniform("uRefraction"), Math.max(config.refraction, 0) * dpr);
...
export function Ripple({ children, className, style, ...options }: RippleProps) {
  ...
  useEffect(() => {
    ...
    if (!source || !content || !output || reducedMotion) return;
  ...
  }, [initialOptions, native, reducedMotion]);
